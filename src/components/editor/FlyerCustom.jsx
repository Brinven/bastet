...
export default function FlyerCustom({ element }) {
  const { customFields, fields, fonts, resolvedPalette } = useEditor()
  ...
  const labelFill = resolveColor(element.labelFill ?? 'role:inkSoft', resolvedPalette)
  const valueFill = resolveColor(element.fill ?? 'role:body', resolvedPalette)
